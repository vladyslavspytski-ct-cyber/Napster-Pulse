...
const DynamicChartCard = ({ chart, index, isWide }: DynamicChartCardProps) => {
  const renderChart = () => {
        ...
        <BarChart data={chartData} barGap={2}>
          ...
          {data.datasets.map((ds) => (
            <Bar
              ...
              radius={[4, 4, 0, 0]}
              opacity={ds.label === "Average" ? 1 : 0.5}
            />
          ))}
          <Tooltip contentStyle={CustomTooltipStyle} />
          <Legend iconType="circle" wrapperStyle={{ fontSize: "12px" }} />
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[280px] gap-4">
        <div className="relative">
          <div className="text-6xl font-bold text-foreground">{avg}</div>
          <div className="absolute -top-2 -right-6 text-sm text-muted-foreground font-medium">/100</div>
        </div>
        {(min !== undefined || max !== undefined) && (
          <div className="flex gap-6 text-sm text-muted-foreground">
            {min !== undefined && (
              <span>Min: <span className="font-semibold text-accent">{min}</span></span>
            )}
            {max !== undefined && (
              <span>Max: <span className="font-semibold text-interu-mint">{max}</span></span>
            )}
          </div>
        )}
      </div>
  ...
  return (
    <motion.div
      ...
      className={`group relative rounded-2xl border border-border/60 bg-card/80 backdrop-blur-sm overflow-hidden transition-shadow duration-300 hover:shadow-lg ${
    ...
    >
      ...
      <div
        ...
        style={{
          background: `linear-gradient(90deg, ${DATASET_COLORS.Average.stroke}, ${DATASET_COLORS.Median.stroke})`,
          opacity: 0.4,
        }}
      />
      ...
      <div className="p-5 md:p-6">
        <div className="mb-4">
          <h3 className="text-base font-semibold text-foreground">{chart.chart_name}</h3>
